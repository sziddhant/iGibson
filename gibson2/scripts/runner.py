...
class Runner(object):
    ...
    def __init__(self, net, env, num_envs, n_stack, rollout_size=5, num_updates=2500000, max_grad_norm=0.5,
        ...
        self.storage = RolloutStorage(self.rollout_size, self.num_envs, self.ob .shape[0:-1],
    ...
    def train(self):
        ...
        """Environment reset"""
        obs = self.env.reset()
        # obs=np.array([self.env.robots[0].get_position()[0],self.env.robots[0].get_position()[1],self.env.robots[0].=.get_rpy()[2]])
